 ...
 * @constructor
 */
function Client() { process.stdout.write('Server start' + '\n');
  ...
  consolelog('testmode is: ' + (this.testmode ? 'active' : 'inactive'));
   ...
   var hostserver = http.createServer( (request, response) => {
                          if(request.method == 'POST'){
                            //this.host_send("entered post");
                            let body = [];
                            request.on('data', (chunk) => {
                                body.push(chunk);
                            }).on('end', () => {
                                body = Buffer.concat(body).toString();
                                this.host_handle_message(body);
                                //this.host_send(body);
                            })
                          }
                         response.end(JSON.stringify(this.message) + '\n');
                       });
  ...
  try {
    ...
    if (msg) {
      ...
      if (msg.action === 'key') {
        ...
        if (!this.shouldKill) {
          this.host_send('ENTERED KILLING STATE')
